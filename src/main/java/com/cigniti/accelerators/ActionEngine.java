...
import com.cigniti.utilities.ErrorUtil;
...
public class ActionEngine extends TestEngine {
	...
	public boolean click(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("Click "+locatorName, "Unable to clicked on " + locatorName);
				return true;
			} else if (b && flag) {
	...
	public boolean type(By locator, String testdata, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("Enter Text Into "+locatorName,
						"Data typing action is not perform on " + locatorName + " with data is " + testdata);
				return true;
			} else if (b && flag) {

				SuccessReport("Enter Text Into "+locatorName,
						"Data typing action is performed on " + locatorName + " with data is " + testdata);
	...
	public boolean switchWindowByTitle(String windowTitle, int count) throws Throwable {
		...
		try {
			...
			if (!flag) {
				...
				SuccessReport("SelectWindow ", "Focus navigated to the window with title " + windowTitle);
			}
		}
	}
	...
	public boolean assertTextPresent(String text) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("AssertTextPresent ", text + " present in the page ");
				return false;
			} else if (b & flag) {
	...
	public boolean asserTitle(String title) throws Throwable {
		...
		}catch(Throwable e)
		{
			...
			if(!flag)
			{
				failureReport("AsserTitle ", "Page title mismatched" );
				flag=false;
				return false;
			} else if (b && flag) {
...
*/
	...
	public String getText(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				warningReport("GetText from "+ locatorName, " Unable to get Text from " + locatorName);
			} else if (b && flag) {
				SuccessReport("GetText from "+ locatorName, " Able to get Text from " + locatorName+" and the text is"+text);
			}
		}
		return text;
	}
	...
	public boolean switchToFrameByLocator(By lacator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("SelectFrame ", " The Frame " + locatorName + " is not selected");
			} else if (b && flag) {
	...
	public boolean hitKey(By locator, Keys keyStroke, String locatorName) throws Throwable {
		...
		try {
			...
			if (flag) {
				...
				failureReport("Type ",
						" Data typing action is not perform on" + locatorName + " with data is " + keyStroke);
	...
	public void failureReport(String strStepName, String strStepDes) throws Throwable {
		...
		case 2:
			...
			//screenShot(TestEngine.filePath() + strStepDes.replace(" ", "_") + ".jpeg");
			flag = true;
			onFailure(strStepName, strStepDes);
			break;
